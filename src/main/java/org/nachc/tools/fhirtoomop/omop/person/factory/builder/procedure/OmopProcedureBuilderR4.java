...
import org.nachc.tools.fhirtoomop.omop.person.factory.builder.condition.translate.OmopConditionFromProcedureR4;
import org.nachc.tools.fhirtoomop.omop.person.factory.builder.observation.translate.OmopMeasurementFromProcedureR4;
import org.nachc.tools.fhirtoomop.omop.person.factory.builder.observation.translate.OmopObservationFromProcedureR4;
...
public class OmopProcedureBuilderR4 {
	...
	public OmopProcedureBuilderR4(FhirPatient fhirPatient, OmopPerson omopPerson, Connection conn) {
	...
	private void addProcedureOccurenceDvo(ProcedureParser proc) {
		...
		ConceptDvo procConcept = FhirToOmopConceptMapperR4.getOmopConceptForFhirCoding(procCoding, conn);
		if (procConcept == null) {
	...
	private void addAsMeasurement(ProcedureParser proc, ConceptDvo procConcept) {
		MeasurementDvo dvo = new OmopMeasurementFromProcedureR4(fhirPatient, proc, procConcept, omopPerson, conn).build();
		this.measurementList.add(dvo);
	}

	private void addAsObservation(ProcedureParser proc, ConceptDvo procConcept) {
		ObservationDvo dvo = new OmopObservationFromProcedureR4(fhirPatient, proc, procConcept, omopPerson, conn).build();
		this.observationList.add(dvo);
	}

	private void addAsCondition(ProcedureParser proc, ConceptDvo procConcept) {
		ConditionOccurrenceDvo dvo = new OmopConditionFromProcedureR4(fhirPatient, proc, procConcept, omopPerson, conn).build();
