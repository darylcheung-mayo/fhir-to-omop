...
import org.nachc.tools.fhirtoomop.fhir.parser.r4.observation.component.ObservationComponentParser;
...
import org.nachc.tools.fhirtoomop.omop.person.factory.builder.observation.translate.OmopMeasurementFromObservationR4;
...
import org.nachc.tools.fhirtoomop.util.mapping.impl.FhirToOmopConceptMapperR4;
...
@Slf4j
public class OmopObservationBuilderR4 {
	...
	private List<OmopMeasurementFromObservationR4> measurementList = new ArrayList<OmopMeasurementFromObservationR4>();
	...
	public OmopObservationBuilderR4(FhirPatient fhirPatient, OmopPerson omopPerson, Connection conn) {
	...
	private List<MeasurementDvo> getMeasurementList() {
		...
		for (OmopMeasurementFromObservationR4 meas : this.measurementList) {
	...
	private ObservationDvo buildSingleObservation(ObservationParser parser) {
		...
		ConceptDvo obsConceptDvo = FhirToOmopConceptMapperR4.getOmopConceptForFhirCoding(obsCoding, conn);
		...
		if (valueCoding != null) {
			ConceptDvo valueConceptDvo = FhirToOmopConceptMapperR4.getOmopConceptForFhirCoding(valueCoding, conn);
		...
		if (isMeasurement(parser)) {
			...
			fixMeas(parser, dvo);
			OmopMeasurementFromObservationR4 translator = new OmopMeasurementFromObservationR4(parser, null, dvo, conn);
	...
	private void buildMultipleObservations(ObservationParser parser) {
		...
		for (ObservationComponentParser comp : comps) {
			...
			if (isMeasurement(parser)) {
				...
				VisitOccurrenceDvo visit = this.omopPerson.getVisitOccurrenceByFhirId(parser.getEncounterId());
				dvo.setVisitOccurrenceId(visit.getVisitOccurrenceId());
				OmopMeasurementFromObservationR4 translator = new OmopMeasurementFromObservationR4(null, comp, dvo, conn);
				this.measurementList.add(translator);
			} else {
				this.observationList.add(dvo);
			}
	...
	private ObservationDvo getBasicInformation(ObservationParser parser, boolean isSingle) {
		...
		if (dvo.getObservationDate() == null && parser.getEncounterId() != null) {
	...
	private ConceptDvo getUnits(String unitsSystem, String unitsCode, Connection conn) {
		if (unitsSystem == null || unitsCode == null) {
			...
			ConceptDvo unitsConceptDvo = FhirToOmopConceptMapperR4.getOmopConceptForFhirCoding(unitsSystem, unitsCode, conn);
			return unitsConceptDvo;
		}
	}
